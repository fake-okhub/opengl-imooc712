...
class MyGLRenderer(context: Context) : GLSurfaceView.Renderer {
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        mTextureRender = TextureRender(mContext, bitmap, lutBitmap)
    }
    ...
    private fun loadImage() : Bitmap {
        ...
        return BitmapFactory.decodeResource(mContext.resources, R.drawable.bgirl, options)
    }

    private fun loadLutImage(): Bitmap {
        val options  = BitmapFactory.Options()
        options.inScaled = false
        return BitmapFactory.decodeResource(mContext.resources, R.drawable.lut_d, options)
    }
